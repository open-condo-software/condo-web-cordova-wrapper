		...
		condo: {
			requestServerAuthorizationByUrl(url, _options, success, error) {
				wrapPromiseWithCallbacks(
					sendCordovaMessage('condo-bridge', 'CondoWebAppRequestAuth', { url }, 10_000).then((data) => data?.response),
					success,
					error,
				)
			},
...
function setupResizeObserver() {
	const observer = new ResizeObserver((entries) => {
		if (entries && entries.length) {
			sendCordovaMessage('condo-bridge', 'CondoWebAppResizeWindow', { height: entries[0].target.clientHeight }).catch(
				(error) => {
